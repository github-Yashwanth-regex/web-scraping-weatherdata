  ...
  observeEvent(input$update, {
    ...
    output$temp_plot <- renderPlotly({
      plot_ly(combined_data) %>%
        add_lines(x = ~Date, y = ~Temperature, color = ~DataType, colors = c('Historical' = 'blue', 'Forecasted' = 'green', 'Current' = 'red'), name = 'Temperature') %>%
        add_markers(data = subset(combined_data, DataType == 'Current'), x = ~Date, y = ~Temperature, color = I("red")) %>%
        layout(title = paste("Temperature Over Days in", city),
               xaxis = list(title = "Date"),
               yaxis = list(title = "Temperature (°C)")) %>%
        add_lines(data = combined_data, x = ~Date, y = ~Temperature, color = ~DataType, colors = c('Historical' = 'blue', 'Forecasted' = 'green', 'Current' = 'red'), line = list(dash = 'dot', width = 1))
    })
    
    # Pressure plot
    output$pressure_plot <- renderPlotly({
      plot_ly(combined_data) %>%
        add_lines(x = ~Date, y = ~Pressure, color = ~DataType, colors = c('Historical' = 'blue', 'Forecasted' = 'green', 'Current' = 'red'), name = 'Pressure') %>%
        add_markers(data = subset(combined_data, DataType == 'Current'), x = ~Date, y = ~Pressure, color = I("red")) %>%
        layout(title = paste("Pressure Over Days in", city),
               xaxis = list(title = "Date"),
               yaxis = list(title = "Pressure (hPa)")) %>%
        add_lines(data = combined_data, x = ~Date, y = ~Pressure, color = ~DataType, colors = c('Historical' = 'blue', 'Forecasted' = 'green', 'Current'= 'red'), line = list(dash = 'dot', width = 1))
    })
    # Humidity plot
    output$humidity_plot <- renderPlotly({
      plot_ly(combined_data) %>%
        add_lines(x = ~Date, y = ~Humidity, color = ~DataType, colors = c('Historical' = 'blue', 'Forecasted' = 'green', 'Current' = 'red'), name = 'Humidity') %>%
        add_markers(data = subset(combined_data, DataType == 'Current'), x = ~Date, y = ~Humidity, color = I("red")) %>%
        layout(title = paste("Humidity Over Days in", city),
               xaxis = list(title = "Date"),
               yaxis = list(title = "Humidity (%)")) %>%
        add_lines(data = combined_data, x = ~Date, y = ~Humidity, color = ~DataType, colors = c('Historical' = 'blue', 'Forecasted' = 'green', 'Current' = 'red'), line = list(dash = 'dot', width = 1))
    })
    
    # Piecharts
    output$temp_pie <- renderPlotly({
      plot_ly(combined_data) %>%
        add_pie(values = ~Temperature, labels = ~paste(Date, "-", DataType), type = 'pie') %>%
        layout(title = "Temperature Distribution")
    })
    
    output$pressure_pie <- renderPlotly({
      plot_ly(combined_data) %>%
        add_pie(values = ~Pressure, labels = ~paste(Date, "-", DataType), type = 'pie') %>%
        layout(title = "Pressure Distribution")
    })
    
    output$humidity_pie <- renderPlotly({
      plot_ly(combined_data) %>%
        add_pie(values = ~Humidity, labels = ~paste(Date, "-", DataType), type = 'pie') %>%
    ...
    output$weather_table <- renderDataTable({
      combined_data$Date <- gsub("\\(.*\\)", "", combined_data$Date)  # Remove the brackets and text within
      combined_data
    }, options = list(
      dom = 'lrtip',  # Hide search box and "Show N entries"
      columnDefs = list(
    ...
    output$temp_bar <- renderPlotly({
      plot_ly(combined_data, x = ~Date, y = ~Temperature, type = 'bar', color = ~DataType, colors = c('Historical' = 'blue', 'Forecasted' = 'green', 'Current' = 'red')) %>%
    ...
    output$pressure_bar <- renderPlotly({
      plot_ly(combined_data, x = ~Date, y = ~Pressure, type = 'bar', color = ~DataType, colors = c('Historical' = 'blue', 'Forecasted' = 'green', 'Current' = 'red')) %>%
    ...
    output$humidity_bar <- renderPlotly({
      plot_ly(combined_data, x = ~Date, y = ~Humidity, type = 'bar', color = ~DataType, colors = c('Historical' = 'blue', 'Forecasted' = 'green', 'Current' = 'red')) %>%
